...
export class FluxService {
  ...
  static async getById(id: number): Promise<FluxReadDto> {
    ...
    const response = await axiosInstance.get<FluxReadDto>(
      `${this.BASE_URL}/${id}`
    );
    return response.data;
  }
  ...
  static async create(flux: FluxCreateDto): Promise<FluxReadDto> {
    // TODO: Implémenter l'appel API pour créer un flux
      ...
      const response = await axiosInstance.post<FluxReadDto>(
        this.BASE_URL,
        flux
      );
      return response.data;
  ...
  static async update(
    id: number,
    flux: Partial<FluxCreateDto>
  ): Promise<FluxReadDto> {
    // TODO: Implémenter l'appel API pour mettre à jour un flux
    const response = await axiosInstance.put<FluxReadDto>(
      `${this.BASE_URL}/${id}`,
      flux
    );
    return response.data;
