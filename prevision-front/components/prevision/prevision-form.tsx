...
import {
  ...
  getWeekDates,
  formatDate,
  getNext6WeeksSundays,
  getWeeksRangeBasedOnDay,
...
export default function PrevisionForm() {
  ...
  const handleFermeChange = async (fermeId: string) => {
    ...
    try {
      setLoading(true);
      const cyclesData = await CycleService.getCyclesByFerme(id);

      setCycles(cyclesData);
    } catch (error) {
      toast.error("Erreur lors du chargement des cycles de la ferme");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCycleChange = async (cycleId: string) => {
    try {
      setLoading(true);
      if (formData.fermeId) {
        const secteurData = await SecteurService.getSecteursByFermeAndCycle(
          formData.fermeId,
          parseInt(cycleId)
        );
      ...
      setLoading(false);
       toast.error("Erreur lors du chargement des secteurs de la ferme");
      console.log(err);
    } finally {
      setLoading(false);
    }
  ...
  const loadVersions = async () => {
    try {
      setLoading(true);
      const data: SystemVersionReadDto =
        await SystemVersionService.getSystemVersionsByFarmId(formData.fermeId);
      setVersions(data);
    } catch (error) {
  ...
  const handleVersionChange = (versionId: string) => {
    ...
    if (formData.semaine) {
      ...
      if (version) {
        ...
        const filteredDates = weekDates.slice(
          versionData.startDay,
          versionData.endDay + 1
        );
  ...
  const getNextStepMessage = () => {
    ...
    if (!formData.typePrevision)
      return "Veuillez sélectionner un type de prévision";
    if (formData.typePrevision === "hebdomadaire" && !formData.semaine)
      return "Veuillez sélectionner une semaine";
    if (formData.typePrevision === "hebdomadaire" && !formData.versionId)
      return "Veuillez sélectionner une version";
    return null;
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const details: PrevisionDetailsCreateDto[] = secteurs.map((secteur) => {
        const lignesPrevision: LignePrevisionCreateDto[] = gridDates.map(
          (date, dateIndex) => {
            console.log(date);
                    ...
                    {gridDates.map((_, dateIndex) => (
                      ...
                      >
                        <input
                          ...
                          placeholder="0"
                          value={
                            gridValues[`${secteur.code}-${dateIndex}`] || ""
                          }
                          onChange={(e) =>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Select
              value={formData.fermeId?.toString() || ""}
              onValueChange={handleFermeChange}
              disabled={loading}
            >
              <SelectTrigger>
              ...
              >
                ...
                <SelectContent>
                  {cycles.map((cycle) => (
                    <SelectItem
                      key={cycle.codeCycle}
                      value={cycle.codeCycle?.toString() || ""}
                    >
                      {cycle.designation}
                    </SelectItem>
                  ))}
          ...
          {formData.cycleId && (
            <div className="space-y-2">
              ...
              <Select
                value={formData.typePrevision || ""}
                onValueChange={(value) =>
                  handleTypePrevisionChange(value as TypePrevision)
                }
              >
                <SelectTrigger>
          ...
          {formData.typePrevision === "hebdomadaire" && (
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={handleSemaineChange}
              >
                <SelectTrigger>
