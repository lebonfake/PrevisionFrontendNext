...
export default function ValidateursList() {
  ...
  const [filteredValidateurs, setFilteredValidateurs] = useState<
    ValidateurReadDto[]
  >([]);
  ...
  const [validateurToDelete, setValidateurToDelete] =
    useState<ValidateurReadDto | null>(null);
  ...
  const toggleValidateur = async (validateurId: number) => {
    try {
      const response = await validateurService.toggleValidateur(validateurId);
      if (response.status == 200) {
        loadValidateurs();
        toast.success("Succès", {
          description: "Validateur modifé avec succès",
        });
      }
    } catch (err) {
      console.log(err);
       toast.success("Erreur", {
          description: "Une erreur est survenue ressayez plus tard",
        });
    }
  };
            ...
            >
              <CardHeader className="pb-3">
                <CardTitle className="text-lg flex items-center justify-between">
                  ...
                  <Switch
                    id={validateur.id.toString()}
                    checked={validateur.isActive}
                    onCheckedChange={() => toggleValidateur(validateur.id)}
                  />
                  {/*<Button
                    variant="ghost"
                    size="icon"
